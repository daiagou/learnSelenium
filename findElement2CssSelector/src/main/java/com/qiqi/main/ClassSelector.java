...
public class ClassSelector {
    ...
    public static void main(String[] args) {
        ...
        System.setProperty("webdriver.chrome.driver", ClassSelector.class.getClassLoader().getResource("mac/chromedriver").getPath());
        //创建谷歌浏览器驱动
        WebDriver webDriver = new ChromeDriver();
        //访问百度
        webDriver.get("https://passport.baidu.com/v2/?reg");
        //元素的className可能会增加或者减少，尽量找其中不变的。比如下面这个。
        List<WebElement> userNames = webDriver.findElements(By.cssSelector(".pass-text-input-userName"));
        ...
        for(WebElement userName:userNames){
            ...
            if(userName.getAttribute("placeholder")!=null&&userName.getAttribute("placeholder").equals("请设置用户名")){
                userName.sendKeys("ClassSelector");//填上findByName
            }
        }
